...
exports.getPlantById = async (id, userId = null) => {
	...
	if (error && error.code !== 'PGRST116') {
		logger.dbError(error.message, 'plants', 'getById');
		logger.dbResponse('plants', 'getById', false, dbMs);
		throw new Error(error.message);
	}
	logger.dbResponse('plants', 'getById', true, dbMs);
	return data || null;
};
